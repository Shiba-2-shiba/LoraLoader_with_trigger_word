...
ComfyUI/
  custom_nodes/
    LoraLoader_with_trigger_word/
      __init__.py
      lora_loader_model_only_trigger_words.py
      pyproject.toml
      README.md
```
...
- `Load Trigger Words`
  ...
  - queue 実行前に、トリガーワードが取得できたかその場で確認できます
...
4. `enable_civitai_fallback=true` のときだけ Civitai `by-hash`
...
埋め込み metadata に十分な情報が無く、`enable_civitai_fallback=true` の場合のみ Civitai fallback を試します。
...
`enable_civitai_fallback=true` のとき、LoRA ファイルの SHA256 を計算し、以下を参照します。
