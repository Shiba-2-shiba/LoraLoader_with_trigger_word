...
- `Trigger Words` / `Browse` 表示タブで queue 前に確認可能
...
- Civitai model card URL を解決して `Browse` タブから開ける
...
- `Trigger Words`
  - 取得結果や失敗理由を表示
- `Browse`
  - Civitai model card URL、model id、version id、解決元を表示
- `Load Trigger Words`
...
URL が解決できた場合は `Open Model Card` ボタンから外部ブラウザで開けます。
